...
\SweaveOpts{keep.source=TRUE, pdf.version=1.4, prefix.string=figs/brainMatching/}
...
@
\end{center}
\caption{Plot of the mean average precision over the range of thresholds tested for the aggregated dataset (approach 3). Solid lines indicates the mean average precision for the pathways (red) and randomly constructed genesets (blue)}
