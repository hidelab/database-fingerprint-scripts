...
POE.matrix<-POE.matrix[grep("Reactome", rownames(POE.matrix)),]
...
@
\end{center}
\caption{Plot of the intra-cluter vs inter-cluster variance ratio over the range of thresholds tested for the aggregated dataset. Variance is defined based on the Manhattan distance. Dashed line indicates the ratio for the unthresholded POE matrix, the barcode produces a ratio of \Sexpr{round(barcode.varRatio.manhattan, 3)}, and gene expression (based on spearman rank) a ratio of \Sexpr{round(ranks.varRatio.spearman, 3)}. The fingerprint at 10E-3 produces a ratio of \Sexpr{round(thresholds.varRatio.manhattan[[which(N == 3)]],3)}, and the unthresholded POE scores a ratio of \Sexpr{round(unthresholded.varRatio.manhattan,3)}.}
...
@
\end{center}
\caption{Plot of the intra-cluter vs inter-cluster variance ratio over the range of thresholds tested for the aggregated dataset. Variance is defined based on the Euclidean distance. Dashed line indicates the ratio for the unthresholded POE matrix, the barcode produces a ratio of \Sexpr{round(barcode.varRatio.euclidean, 3)}, and gene expression (based on spearman rank) a ratio of \Sexpr{round(ranks.varRatio.spearman, 3)}. The fingerprint at 10E-3 produces a ratio of \Sexpr{round(thresholds.varRatio.euclidean[[which(N == 3)]],3)}, and the unthresholded POE scores a ratio of \Sexpr{round(unthresholded.varRatio.euclidean,3)}.}
...
@
\end{center}
\caption{Plot of the average error rate based on 10 repeats of a 5-fold cross-validation over the range of thresholds tested for the aggregated dataset based on the Manhattan distance. Error bars indicate -/+ 1 std.dev. The black line indicates the ratio for the unthresholded POE matrix, and the red for the barcode, dashed lines indicate -/+ 1 std.dev. The barcode produces an error rate of \Sexpr{round(mean(barcode.fiveFold.manhattan), 3)}, SD = \Sexpr{round(sd(barcode.fiveFold.manhattan), 3)}, and gene expression (based on spearman rank) a rate of \Sexpr{round(mean(ranks.fiveFold.manhattan), 3)}, SD = \Sexpr{round(sd(ranks.fiveFold.manhattan), 3)} (N.B. this part is based on distance rather than 1-correlation). The fingerprint at 10E-3 produces a ratio of \Sexpr{round(mean(fiveFold.manhattan[[which(N == 3)]]),3)}, SD = \Sexpr{round(sd(fiveFold.manhattan[[which(N == 3)]]),3)}, and the unthresholded POE scores a ratio of \Sexpr{round(mean(unthresholded.fiveFold.manhattan),3)}, SD = \Sexpr{round(sd(unthresholded.fiveFold.manhattan),3)}.}
...
@
\end{center}
\caption{Plot of the average error rate based on 10 repeats of a 5-fold cross-validation over the range of thresholds tested for the aggregated dataset based on the Euclidean distance. Error bars indicate -/+ 1 std.dev. The black line indicates the ratio for the unthresholded POE matrix, and the red for the barcode, dashed lines indicate -/+ 1 std.dev. The barcode produces an error rate of \Sexpr{round(mean(barcode.fiveFold.euclidean), 3)}, SD = \Sexpr{round(sd(barcode.fiveFold.euclidean), 3)}, and gene expression (based on spearman rank) a rate of \Sexpr{round(mean(ranks.fiveFold.euclidean), 3)}, SD = \Sexpr{round(sd(ranks.fiveFold.euclidean), 3)} (N.B. this part is based on distance rather than 1-correlation). The fingerprint at 10E-3 produces a ratio of \Sexpr{round(mean(fiveFold.euclidean[[which(N == 3)]]),3)}, SD = \Sexpr{round(sd(fiveFold.euclidean[[which(N == 3)]]),3)}, and the unthresholded POE scores a ratio of \Sexpr{round(mean(unthresholded.fiveFold.euclidean),3)}, SD = \Sexpr{round(sd(unthresholded.fiveFold.euclidean),3)}.}
...
abline(h = barcode.LOO.manhattan, lty = 1, col = "red")
@
...
abline(h = barcode.LOO.euclidean, lty = 1, col = "red")
@
\begin{figure}
\begin{center}
...
@
\end{center}
\caption{Plot of the error rate based on a leave-one-out cross-validation over the range of thresholds tested for the aggregated dataset based on the Manhattan distance. The black line indicates the ratio for the unthresholded POE matrix, and the red for the barcode. Error rates are \Sexpr{round(LOO.manhattan[[which(N == 3)]], 3)}, \Sexpr{round(unthresholded.LOO.manhattan, 3)}, \Sexpr{round(barcode.LOO.manhattan, 3)}, and \Sexpr{round(ranks.LOO.manhattan, 3)}.}
...
@
\end{center}
\caption{Plot of the error rate based on a leave-one-out cross-validation over the range of thresholds tested for the aggregated dataset based on the Euclidean distance. The black line indicates the ratio for the unthresholded POE matrix, and the red for the barcode. Error rates are \Sexpr{round(LOO.euclidean[[which(N == 3)]], 3)}, \Sexpr{round(unthresholded.LOO.euclidean, 3)}, \Sexpr{round(barcode.LOO.euclidean, 3)}, and \Sexpr{round(ranks.LOO.euclidean, 3)}.}
...
\label{fig:twelve}
\end{figure}
\subsubsection{Comparison to randomly constructed genesets}
...
@
\end{center}
\caption{Precision-recall curves for aggregated mouse to human tissues data based on a fingerprint build using pathways and functional interaction modules (red) or randomly constructed gene sets (blue).}
\label{fig:thirteen}
\end{figure}
...
#legend("bottomleft", c("Barcode", "Fingerprint (T = 0.001)",
#                   "Fingerprint (T = 0.0001)", "Fingerprint (T = 0.00001)",
#                   "Spearman", "Unthresholded"),
#        text.col = c("blue", "purple", "pink", "red", "green", "black"), bty= "n")
...
@
\end{center}
\caption{Plot of the mean average precision over the range of thresholds tested for the aggregated dataset (approach 3) for the pathway fingerprint (black circles) and a fingerprint build on random gene sets of equivalent size distribution (blue circles). Solid lines indicate the mean average precision for Barcode (blue), Spearman correlation (green) and the unthresholded fingerprint (red)}
\label{fig:MAPplot}
\end{figure}

\begin{figure}
\begin{center}
<<label=randomApproach3,fig=TRUE,echo = TRUE>>=
op<-par(mfrow = c(1,1), pty = "s")
par(mfrow = c(5,6), mar = c(1,1,1,1))
n <- N
for (i in 1:30){
...
@
\end{center}
\caption{Precision-recall curves for aggregated mouse to human tissues data based on a fingerprint build using pathways and functional interaction modules (red) or randomly constructed gene sets (blue) by approach 3.}
\label{fig:randomApproach3}
\end{figure}

\newpage
\section{Figure for manuscript}
For the pathway fingerprinting mauscript we require a figure comparing the precision-recall for the fingerprint, barcode, spearman and random genesets.
\begin{figure}
\begin{center}
<<label=manuscriptFigure,fig=TRUE,echo = TRUE>>=
plot.new()
axis(1, seq(0,1,0.2))
axis(2, seq(0,1,0.2))
title(xlab = "recall", ylab = "precision")
...
#legend(0.5,0.35, c("Barcode", "Fingerprint",
#                   "Spearman", "Random genesets"),
#        text.col = c("blue", "red", "green", "black"), bty= "n")

@
\end{center}
\caption{Precision-recall curves for cross-species tissue data for the Pathway Fingerprint (red, MAP = \Sexpr{round(PR.samples.all[[which(N == 3)]]$MAP, 3)}), unthresholded POE scores (dashed red, MAP = \Sexpr{round(unthresholded.MAP, 3)}), a fingerprint based on random genesets (black, MAP = \Sexpr{round(PR.samples.random.all[[which(N == 3)]]$MAP, 3)}), the gene expression barcode (blue, MAP = \Sexpr{round(attr(PR.samples.barcode.all, "mean.av.precision"), 3)}), and spearman correlation (green, , MAP = \Sexpr{round(spearman.MAP, 3)}).}
\label{fig:PRsamples_barcode_Allplot2}
\end{figure}
