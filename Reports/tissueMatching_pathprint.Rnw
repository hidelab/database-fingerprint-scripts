...
\label{fig:brainHist}
\end{figure}
Figure \ref{fig:brainHist} shows a histogram of the distances from the brain consensus for the full corpus and for the brain tissues that made up the signature. This can also be represented as a table. Or we can extract the top ranking arrays for a particular platform, e.g. what are the closest matched \emph{Danio Rerio}, \emph{Rattus norvegicus}, \emph{Drosophila melanogaster}, \emph{Caenorhabditis elegans} arrays to the human/mouse brain and liver consensus?
<<ZfishBrain>>=
...
print(xtable(Ce.brain.match, caption = "Human/mouse brain - C. elegans matches", label = "Tab_WormBrain",
digits = 3), table.placement = "tbp", caption.placement = "top", size = 5)

print(xtable(Dm.brain.match, caption = "Human/mouse brain - Drosophila matches", label = "Tab_FlyBrain",
digits = 3), table.placement = "tbp", caption.placement = "top", size = 5)
...
print(xtable(Ce.liver.match, caption = "Human/mouse liver - C. elegans matches", label = "Tab_WormLiver",
digits = 3), table.placement = "tbp", caption.placement = "top", size = 5)

print(xtable(Dm.liver.match, caption = "Human/mouse liver - Drosophila matches", label = "Tab_FlyLiver",
digits = 3), table.placement = "tbp", caption.placement = "top", size = 5)

@
\end{landscape}
<<output, echo=FALSE>>=
write.table(Dr.brain.match, file = "Dr.brain.match.csv", quote = F, row.names = T, col.names = T, sep = "\t")
write.table(Rn.brain.match, file = "Rn.brain.match.csv", quote = F, row.names = T, col.names = T, sep = "\t")
write.table(Ce.brain.match, file = "Ce.brain.match.csv", quote = F, row.names = T, col.names = T, sep = "\t")
write.table(Dm.brain.match, file = "Dm.brain.match.csv", quote = F, row.names = T, col.names = T, sep = "\t")
write.table(Dr.liver.match, file = "Dr.liver.match.csv", quote = F, row.names = T, col.names = T, sep = "\t")
write.table(Rn.liver.match, file = "Rn.liver.match.csv", quote = F, row.names = T, col.names = T, sep = "\t")
write.table(Ce.liver.match, file = "Ce.liver.match.csv", quote = F, row.names = T, col.names = T, sep = "\t")
write.table(Dm.liver.match, file = "Dm.liver.match.csv", quote = F, row.names = T, col.names = T, sep = "\t")
@
\pagebreak
...
brain.terms<-c(
  ...
  "Gloubus Pallidum", "Globus pallidus",
