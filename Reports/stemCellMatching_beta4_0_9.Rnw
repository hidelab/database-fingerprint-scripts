...
par(mfcol = c(2,1))
    hist(typeDistance[["ES"]][, "distance"], breaks = seq(0,1,1/(2*sum(abs(typeConsensus[,"ES"])))),
      col = "#00000080", border = "#00000050", xlab = "", main = "")
    hist(typeDistance[["ES"]][cellTypes$GSM[cellTypes$Type == "ES"], "distance"], breaks = seq(0,1,1/(2*sum(abs(typeConsensus[,"ES"])))),
      col = "#FF000080", border = "#FF000020", xlab = "Distance", main = "")
    hist(typeDistance[["ES"]][cellTypes$GSM[cellTypes$Type == "iPS"], "distance"], breaks = seq(0,1,1/(2*sum(abs(typeConsensus[,"ES"])))),
      col = "#00FF0080", border = "#00FF0020", add = TRUE)
@
\newpage
\begin{figure}
...
@
\end{center}
\caption{Histograms of the distance from the cell type consensus fingerprints for ES (red) and iPS (green) and melanoma (blue) samples.}
...
@
\end{center}
\caption{Histograms of the distance from the cell type consensus fingerprints for melanoma (blue), neural crest (red) and mesenchymal (green) samples.}
...
\begin{figure}
\begin{center}
<<label=plotESHist,fig=TRUE,echo=FALSE>>=
<<ESHist>>
@
\end{center}
\caption{Histograms of the distance from the ES consensus fingerprints for all GEO samples (above) and for ES (red) and iPS (green) samples (below)}
\label{fig:ESplot}
\end{figure}
<<defineTerms>>=
...
boxplot(distance ~ Title, data = GSE8840.ES,
